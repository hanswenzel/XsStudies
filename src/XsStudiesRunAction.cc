...
XsStudiesRunAction::XsStudiesRunAction()
    : G4UserRunAction(), numberElastic(0), numberinElastic(0) {
  ...
  analysisManager->CreateH1("diff",
                            "differental cross section scattering angle ", 100,
                            0., 25); // h1 Id = 0
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
...
void XsStudiesRunAction::EndOfRunAction(const G4Run *aRun) {
  ...
  if (IsMaster()) {
    ...
    XsStudiesDetectorConstruction *DetConst =
        (XsStudiesDetectorConstruction *)G4RunManager::GetRunManager()
    ...
    const G4Element *element =
        G4NistManager::Instance()->FindOrBuildElement(elemName);
    ...
    double errtotXs =
        totXs * sqrt(numberElastic.GetValue() + numberinElastic.GetValue()) /
        (numberElastic.GetValue() + numberinElastic.GetValue());
    double errelXs =
        elXs * sqrt(numberElastic.GetValue()) / numberElastic.GetValue();
    double errinelXs =
        inelXs * sqrt(numberinElastic.GetValue()) / numberinElastic.GetValue();
    double inelasticXS = 1.e25 * (store->GetInelasticCrossSectionPerAtom(
                                     glob_particle, glob_energy, element, mat));
    double elasticXS = 1.e25 * (store->GetElasticCrossSectionPerAtom(
                                   glob_particle, glob_energy, element, mat));
    double totalXs = inelasticXS + elasticXS;
    std::cout << "elXs store/calc.:  " << elasticXS << " / " << elXs << "+/- "
              << errelXs << std::endl;
    std::cout << "inelXs store/calc.:  " << inelasticXS << " / " << inelXs
              << " +/- " << errinelXs << std::endl;
    std::cout << "totXs store/calc.:  " << totalXs << " / " << totXs << " +/- "
              << errtotXs << std::endl;
